...
#
# This module outputs the statement as a standalone IAM policy document
# (JSON) so it can be merged — via aws_iam_policy_document.source_policy_documents
# — into whichever resource already owns the assets bucket's
# aws_s3_bucket_policy. It intentionally does NOT create the
# aws_s3_bucket_policy resource itself: a bucket can only be managed by one
# such resource, and that resource almost always already exists (created
